var collegSelectArr = ['电子与通信学院','先进制造学院','电气技术学院','汽车学院','计算机与设计学院','外语商务学院','经济贸易学院','物流学院','马克思主义学院']
var majorSelectArr = [
    ['电子','电信','通信','嵌入','物联'],
    ['数控','模具','机设','机自'],
...
var classSelectArr = [
 ...
 var collegSelect = document.getElementById('collegSelect')
 createOption(collegSelect, collegSelectArr);

 var majorSelect =document.getElementById('majorSelect');
 collegSelect.onchange = function() {
    majorSelect.Options.length = 0;
    createOption(majorSelect,majorSelectArr[collegSelect.value]);
 };

 var classSelect = document.getElementById('classSelect');
 majorSelect.onchange = function() {
     classSelect.options.length = 0;
     createOption(classSelect,classSelectArr[collegSelect.value][majorSelect.value]);
 };

 collegSelect.onchange = function() {
    majorSelect.options.length = 0;
    createOption(majorSelect,majorSelectArr[collegSelect.value]);
    if  (collegSelect.value >= 0) {
